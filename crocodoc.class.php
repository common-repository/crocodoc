...
abstract class Crocodoc {
	...
	*/
	private static function attach_attachment($post_id, $attachment_id, $attachment_title="") {
		...
		$token = get_option('croc-api_key');
            //  Upload to croc.
		$json = self::upload_to_croc(wp_get_attachment_url($attachment_id));
		    //  Get returned data from crocodoc.
		$uuid = $json->{'uuid'};
            //  If Crocodoc upload was successful...
		if ($uuid) {
			    //  Save the attachmentID, returned crocodoc uuid for this uploaded file.
	...
	*/
	private static function get_embeded_doc($postID, $width='500', $height='700' ) {
		...
		if($uuid) {
			...
			$viewURL = "https://crocodoc.com/view/".$sessionId;
			$returnString = '<iframe class="croc_window" src="'.$viewURL.'"';
	...
	/**
	*
	*  Delete the crocodoc uuid, attachment ID, and attachment title for the given post.
	*
	...
	*/
	private static function delete_from_croc($uuid) {
		$token = get_option('croc-api_key');		
		$ch = curl_init("https://crocodoc.com/api/v2/document/delete");
	...
	*  @param string $file  	: URL of the file to upload.
	*  @param string $title		: Title Crocodoc will give the document.
	*  @return json 			: Example {"uuid": "2286b30c-c84d-4516-aaa7-adc7b9aec174"}
	*  @author Michael Doss
	*/	
	private static function upload_to_croc($file) {
		$token = get_option('croc-api_key');		
	...
	*  @author Michael Doss
	*/
	private static function get_session($uuid, $editable='false', $downloadable='false', $admin='false') {
		$token = get_option('croc-api_key');
		$ch = curl_init("https://crocodoc.com/api/v2/session/create");
        $data = http_build_query(array(
           'token'  => $token,
           'uuid' => $uuid,
